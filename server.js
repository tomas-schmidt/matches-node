...
let baseQuery = "select h.team_name as 'homeTeam', select h.team_logo as 'homeTeamLogo', v.team_name as 'visitingTeam', select v.team_logo as 'visitingTeamLogo',  TIME_FORMAT(match_date," + '"%H:%i"' + ") as 'time', DATE_FORMAT(match_date," + '"%d/%m"' + ") as 'day' from matches m join teams h on (h.id_team = m.id_home_team) join teams v on (v.id_team = m.id_visiting_team) join competitions c on (m.id_competition = c.id_competition) where c.competition_name = ";
let todayQuery = "select h.team_name as 'homeTeam', select h.team_logo as 'homeTeamLogo', v.team_name as 'visitingTeam', select v.team_logo as 'visitingTeamLogo', TIME_FORMAT(match_date," + '"%H:%i"' + ") as 'time', DATE_FORMAT(match_date," + '"%d/%m"' + ") as 'day', competition_name from matches m join teams h on (h.id_team = m.id_home_team) join teams v on (v.id_team = m.id_visiting_team) join competitions c on (m.id_competition = c.id_competition) where match_date = (select CURDATE())"
